...
\title{\textbf{Resource-Constrained Cognitive Path Optimization: \\ From Local Plasticity to Low-Cost Predictive Inference}}
\author{Qing Zhou}
\date{}
...
\section{From Free Energy to Cognitive Path Cost}
...
\begin{equation}
\mathcal{C}_{path}[q,\pi]
=
\int_{t_0}^{t_1}
\left[
F_t(q_t)
+
\beta \mathcal{G}_t(\pi_t)
+
\lambda_E E_t
+
\lambda_A A_t
...
E_{\mathrm{base}}
+
\chi(\varepsilon_t^k)^\top\Pi_t^k\varepsilon_t^k
+
E_{\mathrm{plastic}}^k(t),
\end{equation}
...
\section{The Brain Maintains Multiple Candidate Paths}
...
\section{Neural Integration as a Microscopic Basis}
...
\subsection{From LIF to Memory Dynamics: Conditional Derivation}
...
\frac{dW_{ij}}{dt}
=
\eta_W\,\mathrm{pre}_j(t)\,\mathrm{post}_i(t)
-
\lambda_W W_{ij},
\end{equation}
...
F_t
=
\frac{1}{2}\varepsilon_t^\top\Pi_t\varepsilon_t
+
\frac{1}{2}\lambda_W\lVert W\rVert^2,
\end{equation}
then gradient descent on $F_t$ gives the local update
\begin{equation}
\dot W
=
\eta_W\,\Pi_t\varepsilon_t\,\phi(m_t)^\top
-
\eta_W\lambda_W W.
\end{equation}
...
=
\sigma\left(
a_\varepsilon\lVert\Pi_t\varepsilon_t\rVert
+
a_VV_{\mathrm{value},t}
+
a_N\mathrm{Novelty}_t
-
a_EE_{\mathrm{plastic},t}
\right),
\end{equation}
so that
\begin{equation}
\dot W
=
\eta_W q_t\,\Pi_t\varepsilon_t\,\phi(m_t)^\top
-
\lambda_W W.
\end{equation}
...
\begin{equation}
\tau_{\mathrm{th}}
\frac{dV_{\mathrm{th}}}{dt}
=
\kappa_{\mathrm{th}}
\left(
r_t-r_{\mathrm{target}}
\right),
\end{equation}
...
-\alpha b
-
\Gamma_b\frac{\partial F}{\partial b}
+
\gamma V_{value}
-
\eta E
...
Here $b$ is the activation strength of a belief or circuit; $-\partial F/\partial b$ is the free-energy gradient acting on that activation; $V_{value}$ is a value signal; $E$ is energetic or resource cost; $\Gamma_b,\alpha,\gamma,\eta$ are regulatory parameters; and $\xi_t$ is unmodeled noise or fluctuation.
...
-\alpha b
+
\beta \Pi_t\varepsilon_t
+
\gamma V_{\mathrm{value}}
-
\eta E_{\mathrm{act}}
+
\xi_t.
...
-\alpha b
+
\beta\Pi_t\varepsilon(m,W)
+
\gamma V_{\mathrm{value}}
-
\eta E_{\mathrm{act}}(\varepsilon)
+
\xi_t,
\end{equation}
with
\begin{equation}
E_{\mathrm{act}}(\varepsilon)
=
E_{\mathrm{base}}
+
\chi\,\varepsilon^\top\Pi\varepsilon.
\end{equation}
...
=
\mathcal C_i\left(
\{V_j(t),s_j(t),W_{jk}(t),V_{\mathrm{th},j}(t)\}_{j,k\in\Omega_i}
\right).
\end{equation}
...
\section{Memory Is Not Recording, but the Preservation of Low-Cost Paths}
...
\subsection{Memory Dynamics as Temporal Free-Energy Minimization}
...
-\nabla_m F_t
=
\left(\frac{\partial G}{\partial m}\right)^\top
\Pi_t\varepsilon_t
-
\nabla_m C(m_t),
\end{equation}
...
\sigma_m dB_t^i,
\end{equation}
where $a_i(t)$ is recent activation of the trace; $\mathrm{coact}_i(t)$ is local pre-post or circuit-level co-activation; $n_i(t)$ is a neuromodulatory or consolidation signal, such as salience, reward, arousal, sleep-dependent replay, or biological availability for plasticity; and $\lambda_i m_i$ is decay or competition from unused traces.
...
\begin{equation}
\text{repeated co-activation}\uparrow
\quad\Longrightarrow\quad
W_i\uparrow,\ V_{\mathrm{th},i}\ \text{adapts}
\quad\Longrightarrow\quad
\varepsilon_t(o_t,m_i)\downarrow,\ E_{\mathrm{act}}(o_t|m_i)\downarrow.
\end{equation}
Equivalently, at the observer level,
\begin{equation}
E_{\mathrm{act}}(o_t|m_i)
=
E_{\mathrm{base}}
+
\chi[-\ln p(o_t|m_i)],
...
E_{\mathrm{base}}
+
\chi\,\varepsilon_t^\top\Pi_t\varepsilon_t.
\end{equation}
...
\begin{equation}
\text{trace tends to remain reusable}
\quad
\Longleftrightarrow
\quad
V_i^{obs}(t)>0,
...
This also explains why abstraction emerges. Abstraction is not an additional faculty imposed on memory. It is a natural result of memory dynamics under finite resources. When several experiences repeatedly recruit overlapping circuits, those circuits become easier to reactivate and more energetically efficient. At the macro level, this appears as a structure that reduces free energy across situations.

Define the observer-level value of an abstract structure $a$ as
\begin{equation}
V^{obs}(a)
=
\sum_j w_j \Delta F_a^{obs}(e_j)
-
C_{\text{maintain}}(a).
\end{equation}
...
\section{The Brain Does Not Minimize Immediate Effort Alone}
...
E_{\mathrm{base}}
+
\chi\,\varepsilon_C(t|m_C)^\top\Pi_C\varepsilon_C(t|m_C).
\end{equation}
...
\section{Overload, Fatigue, and Attentional Collapse}
...
\section{The Higher-Order Observer as Meta-Control}
...
dz_t^k
&=
\frac{-z_t^k-\ell_t^k}{\tau_z}dt
+
\sigma_z dB_t^{z,k}
\\
\rho_t^k
&=
\frac{\exp(z_t^k/T_{\mathrm{eff}})}{\sum_j\exp(z_t^j/T_{\mathrm{eff}})}
\\
dm_i
&=
\left[
-\lambda_i m_i
+
\alpha_\varepsilon\lVert\Pi_t\varepsilon_i\rVert
+
\alpha_VV_i(t)
+
\alpha_N\mathrm{Novelty}_i(t)
+
\alpha_c\,\mathrm{coact}_i(t)
-
\alpha_EE_i(t)
...
\hat o_t^k
&=
G_k(m_t,\mathbf W_t^k), \\
\varepsilon_t^k
&=
o_t-\hat o_t^k, \\
F_t^k
&=
\frac{1}{2}(\varepsilon_t^k)^\top\Pi_t^k\varepsilon_t^k
+
\frac{1}{2}\lambda_W\lVert\mathbf W_t^k\rVert^2
+
\mathrm{Complexity}_t^k, \\
E_t^k
&=
E_{\mathrm{base}}
+
...
\rho_t^k
=
\frac{\exp(-\ell_t^k/T_{\mathrm{eff}})}
{\sum_j\exp(-\ell_t^j/T_{\mathrm{eff}})}.
\end{equation}
...
\dot{\mathbf W}_t^k
&=
\eta_W q_t^k\Pi_t^k\varepsilon_t^k\phi(m_t)^\top
-
\lambda_W\mathbf W_t^k
+
\sigma_W\xi_W^k, \\
\tau_{\mathrm{th}}\,dV_{\mathrm{th},t}^k
&=
\kappa_{\mathrm{th}}
\left(
r_t^k-r_{\mathrm{target}}
\right)dt.
\end{align}
...
\rightarrow
\text{plasticity and adaptive thresholds}
\\
&\rightarrow
\text{priors}
\rightarrow
\text{lower future error and energy}
\\
&\rightarrow
\text{cognitive path cost}
\rightarrow
...
\section{Potential Empirical Predictions and Operationalization}
...
\exp
\left(
-\frac{\mathcal{C}_k}{T_{\mathrm{eff}}}
\right).
\end{equation}
...
\section{What Can Be Proved and What Remains Hypothetical?}
...
Lieder, F., \& Griffiths, T. L. (2020). Resource-rational analysis: Understanding human cognition as the optimal use of limited computational resources. \textit{Behavioral and Brain Sciences}, 43, e1.
...
Schmidhuber, J. (2009). Driven by compression progress: A simple principle explains essential aspects of subjective beauty, novelty, surprise, interestingness, attention, curiosity, creativity, art, science, music, jokes. \textit{Dagstuhl Seminar Proceedings}.

Shenhav, A., Botvinick, M. M., \& Cohen, J. D. (2013). The expected value of control: An integrative theory of anterior cingulate cortex function. \textit{Neuron}, 79(2), 217--240.
