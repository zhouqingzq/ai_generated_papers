...
\title{\textbf{The Brain as a Least-Action-Like Cognitive System: \\ Cognitive Path Optimization Under Finite Resources}}
\author{Qing Zhou}
\date{}
...
\section{From Free Energy to Cognitive Action}
...
This equation is only the minimal open-loop form. A cognitive circuit that learns cannot keep $I(t)$ and $V_{th}$ fixed. In a predictive-coding circuit, the effective drive is dominated by prediction error rather than raw input alone. Let a lower-level signal be $y_t$ and let a higher-level memory or prior generate the prediction
\begin{equation}
\hat y_t
=
g(m_t,W_t).
...
&=
\left[
\eta_W\varepsilon_t^k\phi(m_t)^\top
-
\lambda_W W_t^k
\right]dt
+
...
\exp
\left(
-\frac{\mathcal{A}_k}{T_{\mathrm{eff}}}
\right).
\end{equation}
