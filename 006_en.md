# Memory as Propagation Terrain: A Computational Theory Inspired by Neural Fields, Replay, Plasticity, and Free Energy
...
**Keywords:** memory dynamics; propagation terrain; neural fields; hippocampal replay; synaptic plasticity; excitation-inhibition balance; free energy; active inference; agent memory; weak-evidence aggregation

---

## 1. What Does Memory Preserve?
...
1. Past experience changes present perception, attention, and action even when it is not explicitly recalled.
2. Repeated experiences form skills, habits, vigilance, and avoidance rather than merely adding records.
3. The same cue can activate different outcomes under different goals, bodily states, and contexts.
4. Memory structure continues to change during sleep and rest, when no new external event is occurring.
5. Behavioral influence often arises from many weak traces acting together rather than from one maximally similar episode.
...
## 2. Theoretical Position: A Middle-Level Computational Model
...
- A memory field is not an independent physical field.
- A neural activity wave is not memory content itself.
- The hippocampus is not literally a computer cache or a capacitor.
- Free energy is not identical to prediction error alone.
- The prefrontal cortex is not a unique central executive.
- The brain is not assumed to solve the equations below exactly.
...
### 3.1 Neural Fields and Population-Level Propagation
...
### 3.3 Plasticity and Complementary Learning Systems
...
### 3.4 Replay, Reactivation, and Offline Learning
...
## 4. Core Concepts: Field, Wave, Path, and Resistance
...
- $W_t^{XY}$ denotes cell-type-specific connectivity kernels;
- $G_t^{XY}$ denotes goal-, attention-, and context-sensitive gating;
- $\Theta_t^X$ denotes adaptive thresholds or local excitability;
- $H_t$ denotes fast episodic traces and replay buffers;
- $\mathcal{B}_t$ denotes modulatory variables including value, risk, and bodily state.
...
### 4.4 Effective Propagation Resistance
...
## 5. Fast Timescale: A Gated Excitatory-Inhibitory Neural Field
...
## 6. Slow Timescale: Value- and Replay-Modulated Plasticity
...
## 7. Replay: From One Experience to Multiple Internal Updates

### 7.1 Replay as Endogenous Input
...
## 8. Free Energy as a Normative Objective, Not a Neural Magic Variable
...
## 9. Unified Dual-Timescale Model
...
1. Milliseconds to seconds: activity propagation, competition, and gating.
2. Seconds to hours: eligibility, rapid binding, and replay selection.
3. Days and beyond: slow consolidation, forgetting, and structural reorganization.
...
## 10. Four Propositions Derived from Propagation Terrain

### Proposition 1: Repeated Success Shortens Convergence Time, with Saturating Returns
...
### Proposition 2: Failure Learning Can Accelerate Risk Recognition
...
### Proposition 3: Conflict Temporarily Increases Gating Investment
...
### Proposition 4: Weak Evidence Can Cross a Decision Threshold at the Field Level
...
## 11. Memory Fields Versus Top-k Retrieval
...
- A deployment failed because environment variables were not checked.
- A paper contained an error because citations were not verified.
- An analysis had to be repeated because units were not checked.
- An automation damaged files because the target path was not confirmed.

The objects differ and may not be near one another in embedding space. Yet all support the same action direction: **verify before producing a high-impact output**.
...
## 12. An Implementable Architecture for Artificial Agents
...
### 12.6 Audit and Homeostatic Constraints
...
## 13. Simulation and Experimental Designs

### 13.1 Neural-Dynamical Simulation
...
### 13.2 Weak-Evidence Aggregation Benchmark
...
### 13.3 Counterfactual Replay Experiment
...
### 13.4 Biological and Behavioral Predictions

If propagation terrain is explanatory, then:

1. Replay strength and related cortical representational change should jointly predict later convergence speed, not only explicit recall.
2. Failure learning should be able to improve risk-cue detection while suppressing dangerous action.
3. Repetition should reduce activation threshold and response time, with saturating improvement.
4. Moderate conflict should increase control-related signals, whereas extreme load may trigger habitual responding.
5. Multiple individually subthreshold but directionally aligned cues should jointly alter choice.
6. Disrupting offline replay should impair cross-context route consolidation, not only episodic detail.
...
## 16. Definitions, Established Mechanisms, and Hypotheses
