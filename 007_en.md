# Language as an Activation-Control Signal for Associative Memory: A Testable Framework from Propagation Terrain to Transformers
...
**Keywords:** language; associative memory; propagation terrain; activation control; modern Hopfield networks; Transformer; attention; prompting; shared knowledge; falsifiability

---

## 1. The Problem: How Can a Small Signal Produce a Complex Internal Change?
...
- Formal language theory studies symbolic composition and generative rules.
- Information theory studies coding, noise, and transmission efficiency.
- Statistical language modeling studies sequence probability and prediction.
- Pragmatics studies speaker intention, common ground, and context.
- Embodied cognition studies the relation of language to perception, action, and bodily state.
...
## 2. Theoretical Position: A Dynamics of Language Acting on Memory
...
- that language is energy rather than information;
- that all language understanding is memory retrieval;
- that Attention corresponds item by item to biological synaptic activity;
- that the Transformer has been proved to be the unique optimal architecture for language;
- that the brain and the Transformer are globally related by a continuous invertible isomorphism;
- or that formal similarity establishes common origin or mechanistic identity.

The proposal is a candidate model that can be compared with alternatives.

---

## 3. From Transmitting Content to Activating Existing Structure

### 3.1 Shared Knowledge Determines Linguistic Compression
...
### 3.2 A Linguistic Cue Is Not the Complete Memory
...
### 3.3 Language Can Open, Close, and Reweight Routes
...
1. **Entry activation:** increasing access to selected memories or concepts.
2. **Constraint and disambiguation:** reducing candidate states inconsistent with context.
3. **Policy configuration:** changing the rules used in subsequent retrieval, reasoning, or action.

---

## 4. A Unified Dynamical Model of Linguistic Activation

### 4.1 State, Memory, and Linguistic Control
...
- $M$ is propagation terrain formed by long-term experience;
- $E_\ell$ encodes linguistic input as a control signal;
- $G$ configures gating from language, goal, and context;
- $\Psi$ is the state-propagation operator;
- $\Pi$ maps internal state to observable output or action.
...
- $T_{\text{conv}}$ is time to a stable output;
- $E_{\text{act}}$ is total activation or computation cost;
- $E_{\text{spread}}$ is irrelevant-route spread;
- $P_{\text{wrong}}$ is the probability of convergence to an incorrect state;
- $H(Z|\ell,c,M)$ is uncertainty over candidate states after the input.
...
### 4.3 A Linguistic-Control Functional
...
- $\mathcal{C}$: structural and computational complexity;
- $\mathcal{I}$: interference of new learning with old routes;
- $\mathcal{R}$: fragility to paraphrase, noise, and context change;
- $\mathcal{B}$: behavioral cost caused by false low-resistance routes, bias, or dangerous habits.
...
### 4.4 Optimal Signals Depend on the Receiver
...
## 5. Associative Memory, Attention, and Transformers

### 5.1 The Formal Relation Between Attention and Modern Hopfield Retrieval
...
- $q$ is the current retrieval cue;
- $K$ defines the addresses or match representations of candidate patterns;
- softmax converts matches into competitive retrieval weights;
- $V$ supplies the content retrieved and written into the current state.
...
### 5.2 Contextual Attention as Dynamically Addressable Short-Term Association
...
### 5.3 Feed-Forward Layers as Parameterized Association and Transformation
...
### 5.4 Residual Streams as Persistent State Transport and Incremental Revision
...
### 5.5 Layered Computation as Iterative Constraint, Not Strict Monotonic Pruning
...
### 5.6 Component-Level Candidate Mapping

| Function in the propagation-terrain framework | Candidate Transformer implementation | Status |
|---|---|---|
...
## 6. Why Prompts Work: Activation, Disambiguation, and Program Configuration

### 6.1 Prompting Is Not Merely Adding Information
...
- $\Delta A_{\text{entry}}$ improves access to relevant knowledge;
- $\Delta A_{\text{disamb}}$ suppresses irrelevant interpretations;
- $\Delta A_{\text{program}}$ configures intermediate computation or output policy;
- $\Delta A_{\text{verify}}$ induces checking of candidate answers.

### 6.2 Few-Shot Examples as Temporary Route Shaping
...
1. Structurally aligned examples with different vocabulary may outperform lexically similar examples with the wrong structure.
2. Mutually conflicting examples should increase route competition and output variance.
3. Example order should change temporary propagation terrain and therefore affect output.
4. When the necessary primitives are absent from long-term parameters, the benefit of few-shot prompting should be bounded.

### 6.3 Reasoning Prompts as Control of Computational Routes
...
## 7. From Linguistic Communication to Shared Propagation Terrain

### 7.1 Common Ground Enables Highly Compressed Communication
...
### 7.2 Dialogue as Online Modeling of Another System's Terrain
...
### 7.3 Metaphor and Analogy as Bridges Between Regions
...
## 8. Distinguishing Theoretical Propositions

### Proposition One: Linguistic Efficiency Increases with Shared Knowledge
...
### Proposition Two: Multiple Subthreshold Cues Can Produce Superadditive Retrieval
...
### Proposition Three: Effective Prompts Primarily Change Routes, Not Merely Token Count
...
### Proposition Four: Contextual-Retrieval and Long-Term-Association Damage Produce Different Deficits
...
### Proposition Five: Language Learning Optimizes Accessibility, Not Only Frequency Fit
...
### 9.1 Shared Knowledge and Minimum Linguistic Length
...
- token count required for a fixed success rate;
- instruction ambiguity;
- convergence time or reasoning steps;
- robustness to paraphrase and noise;
- systematic misunderstanding caused by false shared priors.

If shared structure reduces instruction length beyond what word frequency explains, the result supports Proposition One.

### 9.2 Subthreshold-Cue Aggregation
...
- simple concatenation of multiple cues;
- a model with attention interactions removed;
- top-k vector retrieval;
- an iterative associative-retrieval model;
- random or mutually conflicting cues.
...
### 9.3 Causal-Route Experiments on Prompts

Choose prompts with equivalent factual content but different control structure:

- answer directly;
- list constraints before answering;
- construct a counterexample before answering;
- add irrelevant tokens only;
- use a random paraphrase of equal length.
...
### 9.4 Double Dissociation Between Attention and FFNs

Apply controlled perturbations separately to attention outputs and FFN outputs, then evaluate two task classes:

1. tasks strongly dependent on current-context binding but requiring little long-term knowledge;
2. tasks with simple context but strong dependence on trained facts or transformations.
...
### 9.5 Accessibility-Oriented Training

For small models of equal size, compare four training strategies:

1. random sample order;
2. ordering by knowledge dependency;
3. multiple-entry, multiple-context binding of the same knowledge;
4. multiple-entry binding with replay and interference monitoring.

In addition to perplexity, evaluate:

- successful access under short prompts;
- robustness to synonymous reformulation;
- cross-task transfer;
- old-knowledge retention after learning new material;
- reasoning tokens and compute required for a correct answer.
...
### 9.6 Human Behavioral and Neural Experiments
...
- shared background knowledge reduces linguistic length and response time;
- multiple weak cues produce nonlinear retrieval improvement when presented together;
- effective cues alter candidate competition before the target content is explicitly recalled;
- the same word activates different relational structure under different goals;
- learning and offline consolidation improve later cue efficiency, not only explicit-memory score.
...
## 10. Engineering Principles for Knowledge-Dense Small Models

### 10.1 Optimize Knowledge Entries, Not Only Knowledge Copies
...
- boundaries with adjacent knowledge.

### 10.3 Curriculum Ordering and Replay
...
- old knowledge vulnerable to new-learning interference;
- knowledge with low entry coverage;
- knowledge with high dependency centrality across downstream tasks;
- high-confidence but frequently incorrect knowledge;
- samples with strong conflict between new and old routes.

### 10.4 Distinguish Low Resistance from Correctness
...
## 13. Established Results, Model Definitions, and Hypotheses
...
Geva, M., Schuster, R., Berant, J., & Levy, O. (2021). Transformer feed-forward layers are key-value memories. *Proceedings of EMNLP 2021*, 5484-5495.
